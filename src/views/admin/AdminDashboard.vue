    ...
    <div>
        ...
        <div class="flexElem panelHeader flexResponsive">
            ...
            <div class="flexElem flexResponsive">
                <div class="hasSearch">
                    <input type="text" autocomplete="nope" class="inputControl searchIcon" v-model="search_email" placeholder="Search With Customer Email"
                           @keyup.enter="search('email')">
                    <span class="searchBtn" @click="search('email')"></span>
                </div>
                <div class="hasSearch">
                    <input type="text" autocomplete="nope" class="inputControl searchIcon" v-model="search_id" placeholder="Search With Order Number"
        ...
        <div class="panelContent">
            <div class="table-responsive" v-dragscroll.x="draggable">
                <table class="themeTable fixedLayout">
                    ...
                    <tbody>
                    <tr v-for="(order, oIdx) in orderBy(orders, 'date_created')">
                        <td>
                        ...
                        <td>
                            <popper trigger="click" :options="{placement: 'right'}" style="cursor:pointer;" class="linkColor">
                                <div class="popper">
                        ...
                        <td>
                            <span @mouseover="draggable=false" @mouseout="draggable=true">
                                <b-form-select name="status" v-model="order.status" :value="order.status"
                                               :options="orderStatuses" class="input customInput"
                                               @change="changeStatus(oIdx,order.id, order.status, order.project_id)"
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            getOrders(){
                let self = this;
                let ordersRef = self.fb.ordersCollection.where("status", "==", "Ordered")
                    .where("date_deleted", "==", null);
                ordersRef = self.addSearchQuery(ordersRef);
                ordersRef = ordersRef.orderBy("date_created");
                if(self.start_after) {
            ...
            vdropzoneSuccessMultiple(files, response) {
                ...
                if (response.success == false) {
                    ...
                    uploaded_photos.forEach((asset, aIdx) => {
                        ...
                        self.orderCollagesArray.unshift(self.s3_base_url+"order_collages/"+self.activeOrderId+"/customer_"+asset.name);
            ...
            getCollageImages(orderId) {
                ...
                collageRefDoc.get().then((collagesQuerySnapShot) => {
                    collagesQuerySnapShot.forEach((collage) => {
                        ...
                        let url = self.s3_base_url+"order_collages/"+orderId+"/customer_"+collage_data.name
            ...
            changeStatus(oIdx, orderId, status, project_id) {
                let self = this;
                let orderData = {
                    status: status,
                    date_updated: new Date(),
                ...
                if(status == "Shipped") {
                    self.$swal({
                        ...
                        if (result.value) {
                            ...
                            self.orders[oIdx].status = "Ordered"
                        }
                    });
                } else {
                    self.showLoader = true;
