<template>
    <div>
        ...
        <b-modal ref="addUpdateCouponModal" id="addUpdateCouponModal" title="Upload Finished Mozaix" :hide-footer="true"
        ...
        >
            ...
            <div class="columns is-multiple text-center">
                <div class="column is-12 btnsContainter">
                    <b-button class="customButtonMedium width_200 lightColor" @click="modalAction('hide')" ref="closeBtn">Close</b-button>
                </div>
            </div>
        ...
        <div class="flexElem panelHeader flexResponsive mt-3 mb-3">
            ...
            <div class="flexElem flexResponsive">
                <div class="hasSearch">
                    <input type="text" class="inputControl searchIcon" v-model="search_email" placeholder="Search With Customer Email"
                           @keyup.enter="search('email')">
                    <span class="searchBtn" @click="search('email')"></span>
                </div>

                <div class="hasSearch">
                    <input type="text" class="inputControl searchIcon" v-model="search_id" placeholder="Search With Order Number"
                           @keyup.enter="search('id')" @keypress="isInteger">
                    <span class="searchBtn" @click="search('id')"></span>
                </div>
            </div>
        </div>
        <router-link :to="{name:'AdminViewProject', params: {projectId: projectId}}"
                     title="View Project">
            <span style="font-size: 20px;color: #2f4575; margin-top:3px" v-if="activeProject">
                {{activeProject.title | capitalize}}
            </span>
        </router-link>
                    ...
                    <tr v-for="(order, oIdx) in orderBy(orders, 'date_created', -1)">
                        <td>
                            <span @mouseover="draggable=false" @mouseout="draggable=true">
                               {{order.first_name+ " / "+order.order_no}}
                             </span>

                        </td>
                        <td>
                            <popper trigger="click" :options="{placement: 'right'}" style="cursor:pointer;" class="linkColor" @mouseover="draggable=false" @mouseout="draggable=true">
                                <div class="popper">
                                    <p><strong>Name: </strong>{{order.first_name}}</p>
                                    <p><strong>Email: </strong>{{order.order_email}}</p>
                                    <p><strong>Phone No: </strong>{{order.phone_no ? order.phone_no.replace("+1 ", "") : order.phone_no}}</p>
                                    <p><strong>Address: </strong>{{order.address}}</p>
                                    <p><strong>State: </strong>{{order.region}}</p>
                        ...
                        <td>
                            <span @mouseover="draggable=false" @mouseout="draggable=true">
                               {{order.project_photos_count ? order.project_photos_count: "0"}}
                             </span>
                        </td>
                        <td>
                            <span @mouseover="draggable=false" @mouseout="draggable=true">
                               {{order.date_created.toDate()| moment("ddd, Do MMM  YYYY")}}
                             </span>
                        </td>
                        <td>
                            <span @mouseover="draggable=false" @mouseout="draggable=true">
                        ...
                        <td>
                            <span @mouseover="draggable=false" @mouseout="draggable=true">
                               {{order.total_discount | currency}}
                             </span>
                        </td>

                        <template>
                            <td v-if="getOrderStatus(oIdx) == 'Shipped'">
                                <span class="completed" @mouseover="draggable=false" @mouseout="draggable=true">{{order.status}}</span>
                            </td>
                            <td v-else="">
                                 <span @mouseover="draggable=false" @mouseout="draggable=true">
                                     <b-form-select name="status" :value="getOrderStatus(oIdx)" :ref="'status-'+order.id"
                                                    :options="orderStatuses" class="input customInput"
                        ...
                        <td class="order-status">
                            <router-link :to="{name:'AdminViewProject', params: {projectId: order.project_id}}"
                                         title="View Project">
                                {{order.project_id}}
                            </router-link>
                        ...
                        <td>
                            ...
                            <span class="action action_download" @click="createZip(order.id, order.project_id, oIdx, false)"
                                  style="cursor: pointer" title="Download images zip"></span>
                            <span v-if="order.collage_uploaded" class="action action_download" @click="createZip(order.id, order.project_id, oIdx, true)"
                                  style="cursor: pointer" title="Download Mozaix"></span>
                            <router-link :to="{name:'AdminViewProject', params: {projectId: order.project_id}}"
                                         class="action action_view" title="View Project"></router-link>
                            <span class="action action_forward" @click="uploadCollage(order.id, order.project_id, oIdx)"
                                  style="cursor: pointer" title="Upload Mozaix"></span>
                        </td>
                    </tr>
                   <tr v-if="showLoader == false && orders.length <= 0">
                       <td colspan="5">No orders exists</td>
                   </tr>
                    </tbody>
                </table>
            </div>
...
<script>
    ...
    export default {
        ...
        created(){
            let self = this;
            self.fb.projectsCollection.doc(self.projectId).get().then((projectDocSnapShot) => {
                if(projectDocSnapShot.exists) {
                    self.activeProject = projectDocSnapShot.data();
                    self.activeProject["id"] = self.projectId;
                }
            })
        ...
        methods:{
            ...
            getOrders(){
                let self = this;
                let ordersRef = self.fb.ordersCollection.where("project_id", "==", self.projectId)
                ...
                ordersRef.limit(self.per_page).get().then((ordersSnapShot) => {
           ...
           addSearchQuery(ordersRef){
                let self = this;
               if(self.search_id){
                   ordersRef = ordersRef.where("order_no", "==", parseInt(self.search_id));
               }
               if(self.search_email){
                   ordersRef = ordersRef.where("email_terms", "array-contains", self.search_email.toLowerCase());
               }
               return ordersRef;
           },
            search(type) {
            ...
            vdropzoneSuccessMultiple(files, response) {
                ...
                if (response.success == false) {
                    ...
                    let orderRefDoc =  self.fb.ordersCollection.doc(self.activeOrderId).collection("collages");
                    uploaded_photos.forEach((asset, aIdx) => {
                        batch.set(orderRefDoc.doc(), self.$helpers.addTimestamps(asset, true))
                        self.orderCollagesArray.unshift(asset.url);
                        self.orderCollagesThumbsArray.unshift(self.s3_base_url+"order_collages/"+self.activeOrderId+"/thumb_"+asset.name);
                    })
                    if(uploaded_photos.length > 0) {
                        self.fb.ordersCollection.doc(self.activeOrderId).update({
                            ...
                            self.consoleData("Error updating collage status in order: "+error)
                        })
                    }
            ...
            getCollageImages(orderId) {
                ...
                self.orderCollages = [];
                let collageRefDoc = self.fb.ordersCollection.doc(orderId).collection("collages").where("date_deleted", "==", null);
                collageRefDoc.get().then((collagesQuerySnapShot) => {
                    collagesQuerySnapShot.forEach((collage) => {
                        ...
                        self.orderCollagesArray.push(collage_data.url);
                        self.orderCollagesThumbsArray.push(self.s3_base_url+"order_collages/"+orderId+"/thumb_"+collage_data.name);
            ...
            deleteImage(index) {
                ...
                this.$swal({
                    ...
                    if (result.value) {
                        ...
                        self.fb.ordersCollection.doc(self.activeOrderId).collection("collages").doc(id).update({
            ...
            createZip(orderId, projectId, orderIndex, is_collage) {
                ...
                if(projectId){
                    let assetsArray = [];
                    if(is_collage) {
                        self.fb.ordersCollection.doc(orderId).collection("collages").where("date_deleted", "==", null)
                            .get().then((collagesQuerySnapShot) => {
                            ...
                            if(assetsArray.length > 0) {
                                ...
                                self.downloadZip(assetsArray, projectId+".zip", assetsArray.length, orderIndex);
                            } else {
                                self.fb.ordersCollection.doc(orderId).update({
                                    collage_uploaded: false,
                                }).then(() => {
                                    self.orders[orderIndex].collage_uploaded = false;
                                    self.consoleData("collage status updated in order");
                                }).catch((error) => {
                                    self.consoleData("Error updating collage status in order: "+error)
                                })
                        ...
                        self.fb.projectsCollection.doc(projectId).collection("photos").where("date_deleted", "==", null)
                            ...
                            assetsQuerySnapShot.forEach((asset) => {
                                assetsArray.push({path: asset.data().url, name: asset.data().name})
                            })
                            if(assetsArray.length > 0) {
                                self.orders[orderIndex].downloading = true;
                                $("body").addClass("loaderOpen")
            ...
            downloadZip(files, zipName="default.zip", total_files, orderIndex) {
                ...
                files.forEach((photo, pIdx) => {
                    zip.file(photo.name, self.urlToPromise(photo.path, total_files, orderIndex), { base64: true, binary: true })
                })
                ...
                return new Promise(function (resolve, reject) {
                    JSZipUtils.getBinaryContent(url, function (err, data) {
                        if (err) {
                            ...
                            let downloaded_files = JSON.stringify(self.orders[orderIndex].downloaded);
                ...
                if(newStatus == "Shipped") {
                    self.$swal({
                        ...
                        if (result.value) {
                            ...
                            self.fb.ordersCollection.doc(orderId).update(orderData).then(() => {
                                self.consoleData("order status updated");
                                self.search();
                                const increment = self.fb.firestore.FieldValue.increment(1);
                                self.fb.projectsCollection.doc(project_id).update({
                                    completed_orders: increment
                                }).then(() => {
                                    self.consoleData("completed order incremented");
                                    self.fb.usersCollection.doc(user_id).update({
                                        completed_orders: increment
                                    }).then(() => {
                                        self.consoleData("completed order incremented in users collection");
                                        self.showLoader = false;
                                    }).catch((error) => {
                                        self.showLoader = false;
                                        self.consoleData("error incrementing completed orders in users collection")
                                    });
                                }).catch((error) => {
                                    self.showLoader = false
                    ...
                    self.fb.ordersCollection.doc(orderId).update(orderData).then(() => {
                        self.showLoader = false;
                        self.orders[oIdx].status = newStatus;
